...
* **Tareas:**
  ...
  - `flatpak/com.gnote.calendar.json` manifest + `AppImage` `linuxdeployqt` stub.
...
* **Tareas:**
  ...
  - Flathub CI compila, publica en `flatpak remote-ls flathub | grep com.gnote.calendar`.
  - Actualizar `README.md` badge `Flathub` + `docs/MANUAL.md` sección Flatpak.

* **Entregables:** `flatpak/com.gnote.calendar.json` `6.9`, `flatpak/com.gnote.calendar.metainfo.xml`, `gnote-calendar.flatpak` bundle, PR Flathub mergeado.
...
* **Pruebas:**
  ...
  flatpak run com.gnote.calendar --help
  flatpak run com.gnote.calendar
  # En otro PC ya publicado:
  flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo
  flatpak install flathub com.gnote.calendar -y && flatpak run com.gnote.calendar
  ```

* **Criterio:** `flatpak install flathub com.gnote.calendar` en PC limpio instala y arranca Qt sin `xcb-cursor` manual; `flatpak-builder-lint` 0 errores.
...
* [ ] `flatpak install flathub com.gnote.calendar` en PC limpio (Fase 7).
...
Sem 10-11 Fase7 Flathub        -> com.gnote.calendar 6.9 en Flathub, flatpak install flathub ok
```
